...
func main() {
	...
	for py := 0; py < height; py++ {
		yRat := new(big.Rat)
		yRat.SetInt64(int64(py))
		yRatDivider := new(big.Rat)
		yRatDivider.SetInt64(height*4 + ymin)
		yRat.Quo(yRat, yRatDivider)
		y := float64(py)/height*(ymax-ymin) + ymin
...
func mandelbrot(x float64, y float64) color.Color {
	...
	for n := uint8(0); n < iterations; n++ {
		x, y -= (z*z*z*z - 1) / (4 * z * z * z)
		z -= (z - 1/(z*z*z)) / 4
		if ca(cm(x, y, cm(x, y, cm(x, y, x, y)))) < 1e-6 {
...
func cm(x float64, xi float64, y float64, yi float64) (float64, float64) {
	return x*y - xi*yi, x*yi + xi*y
}

func ca(x float64, y float64) float64 {
