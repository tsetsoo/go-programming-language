...
func parseUnary(lex *lexer) Expr {
	...
	return parsePrimaryPostFixWrapper(lex)
}

func parsePrimaryPostFixWrapper(lex *lexer) Expr {
	expr := parsePrimary(lex)
	_, ok := postFixOperators[lex.token]
	if ok {
		expr = postFixUnary{op: lex.token, x: expr}
		lex.next()
	}
	return expr
}
...
func parseLiteral(lex *lexer) literal {
